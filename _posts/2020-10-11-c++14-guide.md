...
## std::make_unique()
However, `std::make_unique()` has been added to the `unique_ptr` critical change from C++11. This is an important function for making sure the memory is correctly managed, and so it was advised to be manually added if you have C++11 but not C++14.
...
There are no code safety additions to C++14.
